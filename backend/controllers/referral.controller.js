    ...
    try {
      ...
      if (pointsRedeemed && pointsRedeemed > 0) {
        if (user.points < pointsRedeemed) {
          return res.status(400).send({ status: false, message: "Insufficient points" });
        }
      ...
      if (amount >= 500 && user.referredUserId && !user.referralPointsAwarded) {
        ...
        await referrer.save();
        return res.status(200).send({ status: true, message: "Referral points added successfully" });
      }
    // For all other purchases, add 10% of the total amount as points
      user.points += Math.floor(amount * 0.1);
      await user.save();

      res.status(200).send({ status: true, message: "Purchase handled successfully", user });
    } catch (error) {
